...
import { createApp, createView, createDragListener, createKeyShortcatListener, createPanListener, createSvgContainer, createZoomListener } from './create'
import { App, ContextMenu, DragListener, DrawListener, EditListener, EventEmitter, KeyShortcatListener, Listener, PanListener, RootContainer, SelectListener, ShortcutListener, Sidebar, StorageListener, SvgContainer, Toolbar, View, ViewListener, ZoomListener, ZoomPanListener } from './tokens'
import { ISidebar, IEditListener, IListener, ISelectListener, IZoomPanListener, IDrawListener, IEventEmitter, IContextMenu, IShortcutListener, IToolbar, IStorageListener, IViewListener } from './types'
...
export function registerBindings(container: Container) {
    ...
    container.bind(StorageListener).toInstance(IStorageListener).inSingletonScope()
...
export function registerInjections() {
    ...
    injected(ISelectListener, SvgContainer, ShortcutListener, EditListener, DragListener, EventEmitter, StorageListener)
    injected(IDrawListener, SvgContainer, EditListener, SelectListener, DragListener, StorageListener)
    ...
    injected(IListener, ContextMenu, SvgContainer, ShortcutListener, ZoomPanListener, SelectListener, EditListener, StorageListener)
