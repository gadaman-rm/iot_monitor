...
export class StorageListener {
    planeName = ""
    constructor(public svgContainer: SvgContainer, public eventEmitter: EventEmitter) {

        getPlan().then(data => {
            if (data[OK_SYM]) {
                const jsonWidgets = JSON.parse(data.body!.widgets) as GetPlanJsonWidget[]
                if(jsonWidgets as any !== "[]") {
                    jsonWidgets.map(item => {
                        const { is, ...attrs } = item
                        const newWidget = isToWidgets(is as any)
                        if (newWidget) {
                            for (const [key, value] of Object.entries(attrs)) newWidget.setAttribute(key, value as any)
    ...
    emitSaveChange(isSaved: boolean) { this.eventEmitter.emit('storage-change', { type: 'change-storage', name: this.planeName, isSaved }) }
