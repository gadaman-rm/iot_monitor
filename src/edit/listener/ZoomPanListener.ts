...
export class ZoomPanListener {
  ...
  public set onZoomPan(fn: (zoom: number, pan: Point) => void) {
    this.#onZoomPan = fn
  }

  initHandler() {
    this.shortcutListener.addEventListener("shortcut-press", (e) => {
      if (e.type === "reset-zoom-pan") this.defaultZoomPan()
    })
    ...
    this.panListener.onPanMove = (e) => {
      if (e.param?.init) {
        ...
        if (this.#onZoomPan)
          this.#onZoomPan(initZoom, paner(point(e), point(e.param.init as any)))
      }
    }
    this.zoomListener.onZoom = (e) => {
      ...
      if (e.ctrlKey) {
        e.preventDefault()
        const wheelDelta = (
          (e as any)?.wheelDelta ? (e as any).wheelDelta : -e.deltaY
        ) as number
        const newZoom = zoomer({ wheelDelta, mouseCoord, initPan, initZoom })
        if (newZoom && this.#onZoomPan)
          this.#onZoomPan(newZoom.zoom, newZoom.pan)
      }
    }
  }
  defaultZoomPan() {
    if (this.#onZoomPan) this.#onZoomPan(1, { x: 0, y: 0 })
  }
}
